...
Func ClearTheForgottenVale()
	Local $optionsForgottenVale = CloneMap($underworld_fight_options)
...
Func ClearTheFrozenWastes()
	Local $optionsFrozenWastes = CloneMap($underworld_fight_options)
...
Func ClearTheChaosPlanes()
	Local $optionsChaosPlanes = CloneMap($underworld_fight_options)
...
Func ClearSpawningPools($reaper)
	Local $optionsSpawningPools = CloneMap($underworld_fight_options)
...
Func ClearBonePits($reaper)
	Local $optionsBonePits = CloneMap($underworld_fight_options)
...
Func ClearTwinSerpentMountains()
	If Not $ATTEMPT_REAPER_QUESTS Or Not $ENABLE_DEMON_ASSASSIN Then
		...
		Return IsPlayerOrPartyAlive() ? $SUCCESS : $FAIL
	EndIf
	Local $optionsTwinSerpentMountains = CloneMap($underworld_fight_options)
...
Func UnwantedGuests($reaper)
	...
	If Not $ATTEMPT_REAPER_QUESTS Or Not $ENABLE_UNWANTED_GUESTS Then
		...
		Return IsPlayerOrPartyAlive() ? $SUCCESS : $FAIL
	EndIf
	Local $optionsUnwantedGuests = CloneMap($underworld_fight_options)
